...
int main() {
    ...
    int size = 100000000; //배열로 선언하면 스택이라는 아주 좁은 메모리 구역에 데이터가 생성
    // 스택의 기본크기는 8MB이다. -> 거대한 데이터를 다룰 때는 좁은 스택대신 컴퓨터의 전체 RAM을 자유롭게 쓸 수 있는 넓은 평야 같은 '힙' 사용
    size_t bytes = size * sizeof(float);
    // size_t GB 단위로 엄청 커질수 있고, 아주 큰 양수만 담을 수 있는 메모리크기 전용 타입
